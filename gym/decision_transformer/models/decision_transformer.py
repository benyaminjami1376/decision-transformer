...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.embed_timestep = nn.Embedding(max_ep_len, 3*hidden_size)
        self.embed_return = torch.nn.Linear(1, hidden_size)
        self.embed_state = torch.nn.Linear(self.state_dim, hidden_size)
        self.embed_action = torch.nn.Linear(self.act_dim, hidden_size)

        self.embed_ln = nn.LayerNorm(hidden_size*3)
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        ).permute(0, 2, 1, 3).reshape(batch_size, seq_length, 3 * self.hidden_size)
        ...
        # ).permute(0, 2, 1).reshape(batch_size, seq_length)
