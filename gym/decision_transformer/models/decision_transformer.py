...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.predict_action = nn.Sequential(
            *([nn.Linear(hidden_size*2, self.act_dim)] + ([nn.Tanh()] if action_tanh else []))
        )
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        action_preds = self.predict_action(x[:, :2].permute(0, 2, 1, 3).view(batch_size, seq_length, -1))  # predict next action given state
